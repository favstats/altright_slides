...
subtitle: "A Machine Learning Project <br> <br> <font size='6'>Simon Roth^[] & Fabio Votta^[] <br> 2018-20-08</font>"
...
pacman::p_load(dplyr, ggplot2,  ggthemes, xaringan, tidyTX, viridis)
...
> How do Alt-Right and Media interact online 
...
* update three slides
* animate toxicity on text

--

**Data Source**: Wikipedia comments annotated with **toxicity** scores avaible in as [Kaggle Competition](https://www.kaggle.com/c/jigsaw-toxic-comment-classification-challenge/data). Five thousend crowd-workers were involved in labeling different toxicity and sub-types like personal attacks for approx. 160k comments. The dataset is documented [here](https://meta.wikimedia.org/wiki/Research:Detox/Data_Release).

* balanced data to 30.000
* Text Normalization
* logo toxic/Jigsaw
* ask for other interesting datasets
...
![What does a language model learn?](../images/test_all.png)

![](../images/compare_classifier.png)
...
key_dates <- tibble(
    ...
    toxic = c(top, mid, low, mid, top, mid, low, mid, top, mid
              #.4, .4, .4, .4,. 4
              ))
...
toxic_timeline %>%
  ...
  event_toxic() +
  geom_line(aes(color = type), size = 1.4) + 
  theme_hc() +
  ...
  scale_color_manual("Type", values = c(pal[8], pal[4])) +
  # ylim(0, 80000) +
  # guides(color = F, text = F)  +
...
audience_hist_dat %>% 
  mutate(intercepts = ifelse(x < .1, .1, intercepts)) %>% 
  # mutate(intercepts = ifelse(x > .99, 1, intercepts)) %>% 
  ggplot(aes(x = x, y = y, fill = intercepts, group = intercepts)) +
  geom_line(color = NA) +
  geom_ribbon(aes(x = x, ymax = y), ymin = 0, alpha = 0.3) +
  geom_vline(aes(xintercept = intercepts), linetype = "dashed") +
  guides(fill = F) +
  viridis::scale_fill_viridis(option = "D", direction = -1) +
  theme_void() +
  labs(x = "\nPercentage of Posts on Alt-Right pages (0% = Media Only; 100% = Altright Only)", y = "Density\n") +
...
null_data %>% 
  ...
  labs(x = "\nPercentage of Posts on Alt-Right pages (0% = Media Only; 100% = Altright Only)", y = "Predicted Probability: Toxicity\n") +
...
### What does the Alt-Right talk about and how?
...
+ Alt-Right activity spikes on days of Trump administration news and major Terror attacks

* Alt-Right pages exhibit much greater levels of toxicity than Media equivalents
+ There is an overlap between users between Alt-Right and Media pages
    + different behaviour
    + different levels of toxicity 

+ *Alex Jones* and *Paul Joseph Watson* seem to be great influencers
